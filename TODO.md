...
- ~~bug: if the number of columns in a row isn't the same as the header, then the `__index__` and/or `__line__` can get back filled with `None` because that's how `DictReader` treats empty fields (fixed)~~
    - ~~fix: add magic fields `_field_names` and to each row after `DictReader` is already done~~
    - ~~related: might have similar issues if there are too many fields in a data row? check it out while working on this (fixed)~~
